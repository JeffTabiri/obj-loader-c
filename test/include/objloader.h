...
void object_attrib_init(object_attrib_t *attrib) {
  attrib->vertices = NULL;
  attrib->indices = NULL;
  attrib->normals = NULL;
  attrib->num_vertices = 0;
  attrib->num_indices = 0;
  attrib->num_vertex_normals = 0;
  attrib->num_faces = 0;
}
...
static int read_obj_file(char *path, object_attrib_t *attrib) {
  ...
  attrib->vertices = (float *)malloc(attrib->num_vertices * 3 * sizeof(float));
  attrib->normals =
      (float *)malloc(attrib->num_vertex_normals * 3 * sizeof(float));
  attrib->indices = (int *)malloc(attrib->num_faces * 3 * sizeof(int));
